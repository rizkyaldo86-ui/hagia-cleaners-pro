...
export default function AdsDatabase() {
  ...
  const aggregateDataByDate = (campaignData: any[], leadsData: any[]) => {
    ...
    const initializeGroup = (date: string) => {
      if (!grouped[date]) {
         grouped[date] = {
           ...
           closingBed: 0, closingLiving: 0, closingCleanMover: 0, closingAuto: 0, closingLainnya: 0, totalClosing: 0,
           leadBed: 0, leadLiving: 0, leadCleanMover: 0, leadAuto: 0, leadLainnya: 0,
           salesBed: 0, salesLiving: 0, salesCleanMover: 0, salesAuto: 0, salesLainnya: 0, totalSales: 0,
    ...
    leadsData.forEach(row => {
       ...
       const isLainnya = !isBed && !isLiving && !isClean && !isAuto;
       ...
       if (asal.includes('iklan fb') || asal.includes('fb ads') || asal.includes('iklan ig') || asal.includes('ig ads') || asal.includes('iklan wa')) {
         g.leadsIklanWA += 1;
       }
       if (asal.includes('ig wa')) g.leadsIGWA += 1;
    ...
    const aggregatedArray = Object.values(grouped).map(row => {
      ...
      const totalKeseluruhanLeads = g.totalLeadsIklan > 0 ? g.totalLeadsIklan : (g.leadBed + g.leadLiving + g.leadCleanMover + g.leadAuto + g.leadLainnya);
      const crWhatsapp = totalKeseluruhanLeads > 0 ? (g.leadsIklanWA / totalKeseluruhanLeads) * 100 : 0;
      ...
      const rataClosingRate = totalLeadIklan > 0 ? (g.totalClosing / totalLeadIklan) * 100 : 0;
      ...
      return {
        ...
        closingRate: formatPct(rataClosingRate),
        ...
        totalClosingIklanLabel: formatNum(g.totalClosing),
        ...
        totalSalesIklanLabel: formatCurr(g.totalSales),
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-[#0f172a]">
                ...
                {columns.map((col) => (
                  <th key={col.key} className="p-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider whitespace-nowrap group relative">
                    <div 
            ...
            <tbody>
              ...
              {Array.from({ length: Math.max(0, 5 - paginatedData.length) }).map((_, i) => (
                <tr key={\`empty-\${i}\`} className="border-b border-slate-800/50 h-14">
                  ...
                  {columns.map(col => <td key={\`empty-col-\${col.key}\`} className="p-3"></td>)}
                </tr>
              ))}
            </tbody>
          </table>
...
fs.writeFileSync('src/components/AdsDatabase.tsx', content);
