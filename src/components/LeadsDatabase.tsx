...
export default function LeadsDatabase() {
  ...
  const [data, setData] = useState<any[]>(() => {
    const saved = localStorage.getItem('hagia-leads-data');
    if (saved) {
    ...
    { 
      ...
      jamMasuk: "08:00 - 17:00", 
      nama: "Putri", 
      nomorTelepon: "628111755536", 
      asalLeads: "Instagram WA", 
      layanan: "Hagia Bed", 
      templateTeks: "", 
      lokasi: "", 
      junk: "", 
    ...
    { 
      ...
      jamMasuk: "08:00 - 17:00", 
      nama: "AH_Prasetyo", 
      nomorTelepon: "6281218260123", 
      asalLeads: "IKLAN WA", 
      layanan: "Hagia Clean Mover", 
      templateTeks: "", 
      lokasi: "", 
      junk: "", 
      catatan: "", 
  ...
  useEffect(() => {
    localStorage.setItem('hagia-leads-data', JSON.stringify(data));
  }, [data]);
          ...
          complete: (results) => {
            if (results.data && results.data.length > 0) {
              const newData = results.data.map((row: any, idx: number) => {
                 // Try to match headers ignoring case and spaces to be safe
                 const getVal = (possibleNames: string[]) => {
                   const key = Object.keys(row).find(k => {
                     if (!k) return false;
                     const cleanK = k.toString().trim().toLowerCase();
                     return possibleNames.some(p => cleanK.includes(p) || p.includes(cleanK));
                   });
                   return key ? row[key] : '';
                 };

                 return {
                   id: idx + 1000, // ensure new IDs
                   tanggal: getVal(['tanggal', 'date', 'tgl']),
                   jamMasuk: getVal(['jam masuk', 'waktu', 'jam', 'chat']),
                   nama: getVal(['nama', 'name', 'pelanggan']),
                   nomorTelepon: getVal(['nomor', 'telepon', 'whatsapp', 'no hp', 'phone']),
                   asalLeads: getVal(['asal', 'sumber', 'source', 'leads']),
                   layanan: getVal(['layanan', 'service', 'jasa']),
                   templateTeks: getVal(['template', 'teks', 'text']),
                   lokasi: getVal(['lokasi', 'location', 'alamat', 'kota']),
                   junk: getVal(['junk', 'spam', 'jurik', 'jurk']),
                   catatan: getVal(['catatan', 'notes', 'keterangan']),
                   statusFollowUp: getVal(['status', 'follow', 'f/u', 'up']),
                   fuH1: getVal(['fu h+1', 'h+1', 'h1', 'fu h1']),
                   fuH3: getVal(['fu h+3', 'h+3', 'h3', 'fu h3']),
                   closing: getVal(['closing', 'close']),
                   kualitas: getVal(['kualitas', 'quality']),
                   nominalClosing: getVal(['nominal', 'harga', 'total', 'rp']),
                   admin: getVal(['admin', 'cs', 'pic', 'petugas'])
                 }
              }).filter((r: any) => r.nama || r.nomorTelepon || r.tanggal || r.admin);
  ...
  const columns = [
    ...
    { key: 'jamMasuk', label: 'JAM MASUK CHAT' },
    { key: 'nama', label: 'NAMA' },
  ...
  const closedLeads = processedData.filter(row => {
    const isCloseMatch = [row.statusFollowUp, row.closing, row.kualitas].some(val => 
      ['closing', 'close', 'ya'].some(k => String(val).toLowerCase().includes(k))
    );
    const hasNominal = parseInt((row.nominalClosing as string || '0').replace(/[^0-9]/g, '')) > 0;
    return isCloseMatch || hasNominal;
  });
  const totalClosing = closedLeads.length;
  const totalNominalClosing = closedLeads.reduce((sum, row) => {
    return sum + parseInt((row.nominalClosing as string || '0').replace(/[^0-9]/g, '') || '0');
  }, 0);
  const averageClosingRate = totalLeads > 0 ? Math.round((totalClosing / totalLeads) * 100) : 0;
  const aov = totalClosing > 0 ? Math.round(totalNominalClosing / totalClosing) : 0;

  const leadWasted = processedData.filter(row => {
    return [row.junk, row.kualitas, row.catatan, row.statusFollowUp, row.asalLeads, row.lokasi].some(val => {
      const s = String(val || '').toLowerCase();
      return s.includes('spam') || s.includes('loker') || s.includes('luar kota') || s.includes('junk') || s.includes('sampah');
    });
  }).length;

  const layananCounts = processedData.reduce((acc, row) => {
    const l = (row.layanan as string || "").trim();
    const key = l === "" ? "Kosong / Lainnya" : l;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const lokasiCounts = processedData.reduce((acc, row) => {
    const loc = (row.lokasi as string || "").trim();
    const key = loc === "" ? "Kosong / Lainnya" : loc;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const asalLeadsCounts = processedData.reduce((acc, row) => {
    const s = (row.asalLeads as string || "").trim();
    const key = s === "" ? "Kosong / Lainnya" : s;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const kualitasCounts = processedData.reduce((acc, row) => {
    const k = (row.kualitas as string || "").trim();
    const key = k === "" ? "Kosong / Lainnya" : k;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
          {[
            ...
            { label: "Total Nominal", value: formatCurrency(totalNominalClosing), color: "text-indigo-400" },
            { label: "Avg Closing Rate", value: `${averageClosingRate}%`, color: "text-amber-400" },
            { label: "AOV (Avg Order Value)", value: formatCurrency(aov), color: "text-fuchsia-400" },
            { label: "Lead Wasted", value: leadWasted, sublabel: "(Spam, Loker, Luar Kota)", color: "text-rose-400" }
          ].map((metric, idx) => (
            <div key={idx} className="bg-[#1e293b]/60 border border-slate-700/50 rounded-2xl p-3 sm:p-4 flex flex-col justify-center items-center text-center shadow-lg hover:border-indigo-500/30 transition-colors group min-w-0">
               ...
               {metric.sublabel && <span className="text-[8px] sm:text-[9px] text-slate-500 mt-1 uppercase font-bold w-full break-words">{metric.sublabel}</span>}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.keys(layananCounts).length > 0 && (
            <div className="bg-[#1e293b]/40 border border-slate-700/50 rounded-2xl p-4 shadow-lg min-h-[120px]">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Breakdown Layanan</h4>
              <div className="flex flex-wrap gap-2">
                {Object.entries(layananCounts).sort((a,b) => Number(b[1]) - Number(a[1])).map(([name, count], idx) => (
                  <div key={`layanan-${idx}`} className="bg-[#0f172a] border border-slate-700/50 rounded-xl px-3 py-1.5 flex items-center justify-between gap-3 min-w-[140px] flex-1 sm:flex-none">
                    <span className="text-xs text-slate-300 font-medium truncate" title={name}>{name}</span>
                    <span className="text-[10px] font-bold text-slate-900 bg-emerald-400 px-1.5 py-0.5 rounded-md">{count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {Object.keys(lokasiCounts).length > 0 && (
            <div className="bg-[#1e293b]/40 border border-slate-700/50 rounded-2xl p-4 shadow-lg min-h-[120px]">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Breakdown Lokasi</h4>
              <div className="flex flex-wrap gap-2">
                {Object.entries(lokasiCounts).sort((a,b) => Number(b[1]) - Number(a[1])).map(([name, count], idx) => (
                  <div key={`lokasi-${idx}`} className="bg-[#0f172a] border border-slate-700/50 rounded-xl px-3 py-1.5 flex items-center justify-between gap-3 min-w-[140px] flex-1 sm:flex-none">
                    <span className="text-xs text-slate-300 font-medium truncate" title={name}>{name}</span>
                    <span className="text-[10px] font-bold text-slate-900 bg-indigo-400 px-1.5 py-0.5 rounded-md">{count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {Object.keys(asalLeadsCounts).length > 0 && (
            <div className="bg-[#1e293b]/40 border border-slate-700/50 rounded-2xl p-4 shadow-lg min-h-[120px]">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Breakdown Asal Leads</h4>
              <div className="flex flex-wrap gap-2">
                {Object.entries(asalLeadsCounts).sort((a,b) => Number(b[1]) - Number(a[1])).map(([name, count], idx) => (
                  <div key={`asal-${idx}`} className="bg-[#0f172a] border border-slate-700/50 rounded-xl px-3 py-1.5 flex items-center justify-between gap-3 min-w-[140px] flex-1 sm:flex-none">
                    <span className="text-xs text-slate-300 font-medium truncate" title={name}>{name}</span>
                    <span className="text-[10px] font-bold text-slate-900 bg-blue-400 px-1.5 py-0.5 rounded-md">{count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {Object.keys(kualitasCounts).length > 0 && (
            <div className="bg-[#1e293b]/40 border border-slate-700/50 rounded-2xl p-4 shadow-lg min-h-[120px]">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Breakdown Kualitas Lead</h4>
              <div className="flex flex-wrap gap-2">
                {Object.entries(kualitasCounts).sort((a,b) => Number(b[1]) - Number(a[1])).map(([name, count], idx) => (
                  <div key={`kualitas-${idx}`} className="bg-[#0f172a] border border-slate-700/50 rounded-xl px-3 py-1.5 flex items-center justify-between gap-3 min-w-[140px] flex-1 sm:flex-none">
                    <span className="text-xs text-slate-300 font-medium truncate" title={name}>{name}</span>
                    <span className="text-[10px] font-bold text-slate-900 bg-amber-400 px-1.5 py-0.5 rounded-md">{count}</span>
                  </div>
                ))}
              </div>
            </div>
