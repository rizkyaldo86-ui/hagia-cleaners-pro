...
import { Search, Download, Table as TableIcon, RefreshCw, Plus, RotateCcw, RotateCw, CheckSquare, Trash2, ChevronDown, Check, Filter, Link, BarChart2 } from 'lucide-react';
...
export default function CrmBlastDatabase() {
  const schema = MENU_SCHEMAS["crm-blast"];
  ...
  const [data, setData] = useState<any[]>(() => {
    const saved = localStorage.getItem('hagia-crm-blast-data');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('hagia-crm-blast-data', JSON.stringify(data));
  }, [data]);
  ...
  const handleAddRow = () => {
    ...
    columns.forEach((c: any) => newRow[c.key] = "");
  ...
  const handleSyncFromLeads = () => {
    setIsSyncing(true);
    setTimeout(() => {
      const leadsDataStr = localStorage.getItem('hagia-leads-data');
      if (leadsDataStr) {
        try {
          const leadsData = JSON.parse(leadsDataStr);
          const closedLeads = leadsData.filter((lead: any) => 
            lead.closing && 
            String(lead.closing).trim().toLowerCase() !== "" && 
            String(lead.closing).trim().toLowerCase() !== "tidak");
          
          let modified = false;
          const newData = [...data];
          
          closedLeads.forEach((lead: any) => {
            const existingIndex = newData.findIndex(d => d._sourceLeadId === lead.id || (d.nama === lead.nama && d.tanggal === lead.tanggal));
            const mappedData = {
              _sourceLeadId: lead.id,
              tanggal: lead.tanggal || "",
              tanggalClosing: lead.tanggal || "",
              jamMasukChat: lead.jamMasuk || "",
              nama: lead.nama || "",
              noTelp: lead.nomorTelepon || "",
              asalLeads: lead.asalLeads || "",
              layanan: lead.layanan || "",
              templateTeks: lead.templateTeks || "",
              lokasi: lead.lokasi || "",
              catatan: lead.catatan || "",
              kualitas: lead.kualitas || "",
              admin: lead.admin || "",
              statusFollowUp: lead.statusFollowUp || "",
              fuH1: lead.fuH1 || "",
              fuH3: lead.fuH3 || "",
              nominalClosing: lead.nominalClosing || "",
              closingAtauTidak: lead.closing || "Closing",
              closing: "Yes",
              layananClosing: lead.layanan || "",
            };

            if (existingIndex >= 0) {
              const currentStringified = JSON.stringify(newData[existingIndex]);
              const newObj = { ...newData[existingIndex], ...mappedData };
              if (currentStringified !== JSON.stringify(newObj)) {
                newData[existingIndex] = newObj;
                modified = true;
              }
            } else {
              newData.push({ id: Math.random().toString(36).substring(7), ...mappedData });
              modified = true;
            }
          });
          ...
          complete: (results) => {
            if (results.data && results.data.length > 1) {
              ...
              const ALIAS_MAP: Record<string, string[]> = {
                ...
                nominalClosing: ['nominal', 'closing', 'harga', 'total', 'rp']
              };
  ...
  const breakdowns = categoricalCols.map((col: any) => {
  ...
  const handleExport = () => {
    const csvContent = [
      columns.map((c: any) => c.label).join(','),
      ...processedData.map(row => columns.map((c: any) => `"${String(row[c.key] || '').replace(/"/g, '""')}"`).join(','))
    ].join('\n');
    ...
    link.setAttribute('download', `export_crm_blast.csv`);
      ...
      <div className="bg-[#0c1226] border border-slate-800/50 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white tracking-tight">{schema.title}</h2>
              <p className="text-slate-400 text-sm mt-1">
                Sinkronisasi data Closing dari Leads otomatis
              </p>
            </div>
          </div>
          
          <div className="flex flex-col xl:flex-row gap-4 items-center">
             <div className="flex bg-[#121b33] p-1.5 rounded-2xl border border-slate-700/50 xl:w-[400px]">
               ...
               <button 
                 ...
                 disabled={isSyncingSheet || !sheetLink}
                 className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:hover:bg-indigo-600 px-5 py-3 rounded-xl flex items-center gap-2 text-sm font-bold shadow-lg shadow-indigo-500/20 transition-all text-white ml-1 shrink-0"
               >
                 <RefreshCw size={18} className={`${isSyncingSheet ? 'animate-spin' : ''}`} />
                 <span className="hidden sm:inline">{isSyncingSheet ? 'Menarik...' : 'Auto Sync'}</span>
               </button>
             </div>
             <button 
               onClick={handleSyncFromLeads}
               disabled={isSyncing}
               className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-sm font-bold shadow-lg transition-all ${isSyncing ? 'bg-emerald-500/20 text-emerald-400 cursor-not-allowed border border-emerald-500/30' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/20 border border-transparent'}`}
             >
               <RefreshCw size={18} className={isSyncing ? "animate-spin" : ""} />
               {isSyncing ? 'Menyinkronkan...' : 'Sync dari Leads'}
             </button>
          </div>
        </div>
              ...
              {paginatedData.length > 0 ? paginatedData.map((row, idx) => (
                ...
                <tr>
                  <td colSpan={columns.length + 1} className="p-8 text-center text-slate-500">
                    Tidak ada data. Silakan klik "Sinkronisasi dari Leads" untuk memuat data atau "Baris Baru".
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="p-4 border-t border-slate-800/50 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-slate-400">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="px-3 py-1 bg-[#1e293b] rounded-lg hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Sebelumnya
            </button>
            <span className="px-3">
