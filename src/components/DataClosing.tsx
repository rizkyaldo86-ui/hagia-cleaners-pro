...
export default function DataClosing() {
  ...
  useEffect(() => {
    const savedLeads = localStorage.getItem('hagia-leads-data');
    const savedClosingStr = localStorage.getItem('hagia-closing-data');
    let savedClosingMap: Record<string, any> = {};

    if (savedClosingStr) {
      try {
        const savedClosing: any[] = JSON.parse(savedClosingStr);
        savedClosingMap = savedClosing.reduce((acc, row) => {
          if (row.tanggal) acc[row.tanggal] = row;
          return acc;
    ...
    if (savedLeads) {
      try {
        const leadsData: any[] = JSON.parse(savedLeads);
        
        // Group by tanggal
        const grouped = leadsData.reduce((acc, row) => {
          const tgl = row.tanggal || 'Unknown';
          if (!acc[tgl]) acc[tgl] = [];
          acc[tgl].push(row);
          return acc;
        }, {} as Record<string, any[]>);

        const aggregated = Object.entries(grouped).map((entry, idx) => {
          const tgl = entry[0];
          const rows = entry[1] as any[];

          const getCount = (keywords: string[], filterKey: string = 'asalLeads') => {
            return rows.filter(r => {
              const s = String(r[filterKey] || '').toLowerCase();
              return keywords.some(k => s.includes(k));
            }).length;
          };

          const crm = getCount(['blast', 'existing', 'crm']);
          const iklanIG = getCount(['iklan ig', 'ig ads', 'iklan wa']);
          const iklanFB = 0;
          const igWA = getCount(['ig wa', 'ig (wa)', 'instagram wa']);
          const igDMCOM = getCount(['ig dm', 'ig com', 'ig komen', 'ig (dm', 'instagram dm']);
          const tiktokWA = getCount(['tiktok wa', 'tik tok wa', 'tiktok (wa)']);
          const tiktokDMCOM = getCount(['tiktok dm', 'tik tok dm', 'tiktok com', 'tik tok (dm']);
          const youtube = getCount(['youtube']); // general youtube but mostly wa
          const youtubeCOM = getCount(['youtube com', 'youtube komen']);
          const waKosong = getCount(['wa kosong', 'google maps', 'maps', 'gmaps']);
          const fbWA = getCount(['fb wa', 'facebook wa']);
          const fbDMKOM = getCount(['fb dm', 'facebook dm', 'fb (dm', 'fb kom']);
          
          const igBayanganWA = getCount(['ig bayangan wa', 'instagram bayangan wa']);
          const igBayanganDMCOM = getCount(['ig bayangan dm', 'ig bay']);
          const tiktokBayanganWA = getCount(['tiktok bayangan wa']);
          const tiktokBayanganDMCOM = getCount(['tiktok bayangan dm']);
          const youtubeBayanganWA = getCount(['youtube bayangan wa']);
          const youtubeBayanganCOM = getCount(['youtube bayangan com']);
          const facebookBayanganWA = getCount(['fb bayangan wa', 'facebook bayangan wa']);
          const facebookBayanganDMCOM = getCount(['fb bayangan dm', 'facebook bayangan dm']);

          const leadsKeseluruhan = rows.length;
          
          const luarKota = rows.filter(r => {
            const loc = String(r.lokasi || '').toLowerCase();
            const cat = String(r.catatan || '').toLowerCase();
            const kwal = String(r.kualitas || '').toLowerCase();
            return loc.includes('luar kota') || cat.includes('luar kota') || kwal.includes('luar kota');
          }).length;

          const spam = rows.filter(r => {
            const junk = String(r.junk || '').toLowerCase();
            const qual = String(r.kualitas || '').toLowerCase();
            return ['spam', 'loker', 'junk', 'sampah'].some(k => junk.includes(k) || qual.includes(k));
          }).length;

          const leadTerjangkau = leadsKeseluruhan - luarKota - spam;

          const cs1Rows = rows.filter(r => String(r.admin || '').toLowerCase().replace(/\s/g, '') === 'cs1');
          const cs2Rows = rows.filter(r => String(r.admin || '').toLowerCase().replace(/\s/g, '') === 'cs2');

          const getClosed = (rs: any[]) => rs.filter(r => {
            const isC = String(r.closing || '').toLowerCase();
            const isSF = String(r.statusFollowUp || '').toLowerCase();
            const n = parseNumber(r.nominalClosing);
            return isC.includes('close') || isSF.includes('closing') || n > 0;
          });

          const closingCS1List = getClosed(cs1Rows);
          const closingCS2List = getClosed(cs2Rows);

          const closingCS1 = closingCS1List.length;
          const salesCS1 = closingCS1List.reduce((acc, r) => acc + parseNumber(r.nominalClosing), 0);
          const totalLeadCS1 = cs1Rows.length;

          const closingCS2 = closingCS2List.length;
          const salesCS2 = closingCS2List.reduce((acc, r) => acc + parseNumber(r.nominalClosing), 0);
          const totalLeadCS2 = cs2Rows.length;

          const totalClosing = closingCS1 + closingCS2;
          const closingRate = leadTerjangkau > 0 ? ((totalClosing / leadTerjangkau) * 100).toFixed(1) + '%' : '0%';
          const totalSales = salesCS1 + salesCS2;
          const averageOrderValue = totalClosing > 0 ? Math.round(totalSales / totalClosing) : 0;
          
          // Restore manually edited fields like totalRevenue if they exist
          const existingRow = savedClosingMap[tgl];
          const totalRevenue = existingRow?.totalRevenue || ""; 
    ...
    } else if (savedClosingStr) {
       // If no leads data, but we have closing data, just load the closing data
       try {
         setData(JSON.parse(savedClosingStr));
       } catch (e) {}
    }
  }, []);
  ...
  useEffect(() => {
    if (data.length > 0) {
      localStorage.setItem('hagia-closing-data', JSON.stringify(data));
    }
  }, [data]);
          ...
          complete: (results) => {
            if (results.data && results.data.length > 0) {
              const standardKeys = [
                ...
                { key: 'crm', aliases: ['crm'] },
                { key: 'iklanIG', aliases: ['iklan ig', 'ig ads', 'iklan wa'] },
                { key: 'iklanFB', aliases: ['iklan fb', 'fb ads'] },
                { key: 'igWA', aliases: ['ig (wa)', 'ig wa'] },
                { key: 'igDMCOM', aliases: ['ig (dm&com)', 'ig dm'] },
                { key: 'tiktokWA', aliases: ['tik tok (wa)', 'tiktok wa', 'tiktok (wa)'] },
                { key: 'tiktokDMCOM', aliases: ['tik tok (dm&com)', 'tiktok dm', 'tiktok (dm&com)', 'tiktok (dm&kom)'] },
                { key: 'youtube', aliases: ['youtube'] },
                { key: 'youtubeCOM', aliases: ['youtube com'] },
                { key: 'waKosong', aliases: ['wa kosong'] },
                { key: 'fbWA', aliases: ['fb wa'] },
                { key: 'fbDMKOM', aliases: ['fb (dm&kom)', 'fb dm'] },
                { key: 'igBayanganWA', aliases: ['ig bayangan wa'] },
                { key: 'igBayanganDMCOM', aliases: ['ig bayangan dm & com', 'ig bayangan dm'] },
                { key: 'tiktokBayanganWA', aliases: ['tiktok bayangan wa'] },
                { key: 'tiktokBayanganDMCOM', aliases: ['tiktok bayangan dm & com', 'tiktok bayangan dm'] },
                { key: 'youtubeBayanganWA', aliases: ['youtube bayangan wa'] },
                { key: 'youtubeBayanganCOM', aliases: ['youtube bayangan com'] },
                { key: 'facebookBayanganWA', aliases: ['facebook bayangan wa'] },
                { key: 'facebookBayanganDMCOM', aliases: ['facebook bayangan dm & com', 'facebook bayangan dm'] },
                { key: 'leadsKeseluruhan', aliases: ['leads keseluruhan', 'total leads', 'leads'] },
                { key: 'luarKota', aliases: ['luar kota'] },
                { key: 'spam', aliases: ['spam', 'junk'] },
                { key: 'leadTerjangkau', aliases: ['lead terjangkau', 'leads terjangkau'] },
                { key: 'closingCS1', aliases: ['closing cs1', 'closing cs 1'] },
                { key: 'salesCS1', aliases: ['sales cs1', 'sales cs 1', 'sales rp'] },
                { key: 'totalLeadCS1', aliases: ['total lead cs 1', 'total lead cs1'] },
                { key: 'closingCS2', aliases: ['closing cs 2', 'closing cs2'] },
                { key: 'salesCS2', aliases: ['sales cs 2', 'sales cs2'] },
                { key: 'totalLeadCS2', aliases: ['total lead cs 2', 'total lead cs2'] },
                { key: 'closingRate', aliases: ['closing rate'] },
                { key: 'totalClosing', aliases: ['total closing'] },
                { key: 'averageOrderValue', aliases: ['average order value', 'aov'] },
                { key: 'totalSales', aliases: ['total sales'] },
                { key: 'totalRevenue', aliases: ['total revenue'] }
              ];
              ...
              const newData = results.data.map((row: any, idx: number) => {
                 ...
                 return mappedRow;
              }).filter((r: any) => r.tanggal || r.leadsKeseluruhan || r.totalClosing);
  ...
  // Calculate Metrics
  const totalLeads = processedData.reduce((sum, row) => sum + Number(row.leadsKeseluruhan || 0), 0);
  const totalLeadTerjangkau = processedData.reduce((sum, row) => sum + Number(row.leadTerjangkau || 0), 0);
  const totalClosing = processedData.reduce((sum, row) => sum + Number(row.totalClosing || 0), 0);
  const totalSales = processedData.reduce((sum, row) => sum + Number(row.totalSales || 0), 0);
  
  const averageClosingRate = totalLeadTerjangkau > 0 ? Math.round((totalClosing / totalLeadTerjangkau) * 100) : 0;
  const aov = totalClosing > 0 ? Math.round(totalSales / totalClosing) : 0;
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
          {[
            ...
            { label: "Lead Terjangkau", value: totalLeadTerjangkau, color: "text-indigo-400" },
            { label: "Total Closing", value: totalClosing, color: "text-emerald-400" },
            { label: "Total Sales", value: formatCurrency(totalSales), color: "text-fuchsia-400" },
            { label: "AOV (Avg Order Value)", value: formatCurrency(aov), color: "text-purple-400" },
            { label: "Avg Closing Rate", value: `${averageClosingRate}%`, color: "text-amber-400" },
          ].map((metric, idx) => (
